...
import { getNegocios, getNegociosByRubro, searchNegocios } from "@/services/negocios";
...
export default async function Categoria({ params, searchParams }: PageProps) {
    ...
    if (search) {
        // Caso 1: Búsqueda desde la caja del home
        negocios = await searchNegocios(search);
        title = `Búsqueda: "${search}"`;
        backLink = "/";
        backLinkLabel = "Volver al Inicio";
    } else if (slug) {
        // Caso 2: Clic en categorías de la navegación clásica
        ...
        if (categoria) {
            ...
            // Si no es una categoría, verificamos si es un rubro (redirección directa)
        ...
        // Si no hay slug ni búsqueda, enviamos 404
        notFound();
    }

    // Paginación
    const itemsPerPage = 6;
    const totalItems = negocios.length;
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const activePage = Math.max(1, Math.min(currentPage, totalPages || 1));
    const paginatedNegocios = totalPages > 0 
        ? negocios.slice((activePage - 1) * itemsPerPage, activePage * itemsPerPage) 
        : [];
    ...
    const getPageHref = (pageNumber: number) => {
        ...
        if (slug) {
            return `/categoria/${slug}?${queryParams.toString()}`;
        }
        return `/categoria?${queryParams.toString()}`;
    };
    ...
    return (
        <main className="w-full max-w-[1440px] mx-auto px-4 md:px-16 py-12">
            ...
            <section className="flex flex-col lg:flex-row gap-8">
                ...
                {/* Resultados */}
                <ul className="w-full lg:w-4/4 flex flex-col gap-12">
                    {negocios.length === 0 ? (
